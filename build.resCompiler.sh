...
if [ "$(expr substr $(uname -s) 1 10)" == "MINGW32_NT" ]; then 
  ...
  if hash $PLS_GCC 2>/dev/null; then
    ...
    PLS_RSC_SDL_MAIN=' -DNO_STDIO_REDIRECT '`ls $PLS_DIR_SDL/source/*.c`
    PLS_RSC_LIBS=$PLS_RSC_SDL_MAIN' -lSDL -lSDL_image'
    
    # MinGW specific values
    if [ "$(expr substr $(uname -s) 1 10)" == "MINGW32_NT" ]; then
      PLS_RSC_GCC_ARGS='-DWIN32 -D_WIN32 -mconsole -L '$PLS_DIR_SDL'/lib '$PLS_RSC_GCC_ARGS
      PLS_RSC_INCLUDES='-I '$PLS_DIR_SDL'/include -I '$PLS_DIR_SDL'/include/SDL '$PLS_RSC_INCLUDES
